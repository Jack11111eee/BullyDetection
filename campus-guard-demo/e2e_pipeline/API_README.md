...
- **Base URL**：`http://localhost:8000`（默认端口，可通过 `--port` 修改）
...
- [ ] `curl -F 'file=@test.mp4' -F 'sourceId=test-01' http://localhost:8000/api/v1/analyze/start` 能拿到 `taskId`
- [ ] `curl -N http://localhost:8000/api/v1/analyze/{taskId}/stream` 能看到 `event: connected` 和连续的 `event: frame`
